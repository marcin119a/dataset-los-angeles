...
def fetch_cnv(client, table, gene_list):
    ...
    SELECT
        case_barcode,
        ARRAY_AGG(STRUCT(gene_name, copy_number)) AS agg
    FROM `{table}`
...
def fetch_rna(client, table, gene_list):
    ...
    SELECT
        case_barcode,
        ARRAY_AGG(STRUCT(gene_name, fpkm_unstranded)) AS agg
    FROM `{table}`
    WHERE gene_name IN ('{genes_sql}')
    GROUP BY case_barcode
    """
...
def fetch_mirna(client, table, mirna_list):
    ...
    SELECT
        case_barcode,
        ARRAY_AGG(STRUCT(mirna_id, reads_per_million_miRNA_mapped)) AS agg
    FROM `{table}`
...
def fetch_methylation(client, table, probe_list):
    ...
    SELECT
        case_barcode,
        ARRAY_AGG(STRUCT(probe_id, beta_value)) AS agg
    FROM `{table}`
